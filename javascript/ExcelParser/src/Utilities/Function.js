 ...
 * @Description This file contains the reimplementation of Excel/Google Spreadsheets functions.
 * At this point, the file contains only the Google Spreadsheets implementation.
 */
define("Utilities/Function", ["require", "Libraries/formula", "XClasses/XLogger", "XClasses/XTypes", "XClasses/XTypedValue", "Utilities/Util"], function (require, Formula, XLogger, XTypes, XTypedValue, Util) {
    ...
    func.INT = function (/*XApplication*/app, /*Address*/source, /*Boolean*/array, /*Boolean*/range, /*Boolean*/full_range, args) {
        ...
        if (args.length !== 1) {
            ...
            //Less code to write if we treat everything as a matrix
            if (!array) {
                val = [
                    [val]
                ];
            }
            //Iterate through the matrix and floor the values
    ...
    func.SUM = function (/*XApplication*/app, /*Address*/source, /*Boolean*/array, /*Boolean*/range, /*Boolean*/full_range, args) {
        ...
        var AST = require("Parser/AST/AST");
        if (args.length === 0) {
            return func._returnError(new XTypedValue("#N/A", XTypes.Error), array);
        }
        for (k = 0; k < args.length; k++) {
            val = args[k].compute(app, source, array, true, true);
            rng = args[k] instanceof AST.ReferenceExpr && args[k].Ref instanceof AST.ReferenceRange;
            if (!(val instanceof Array)) {
                val = [
                    [val]
                ];
    ...
    func.ARRAYFORMULA = function (/*XApplication*/app, /*Address*/source, /*Boolean*/array, /*Boolean*/range, /*Boolean*/full_range, args) {
        if (args.length !== 1) {
            ...
            if (array || full_range || range) {
                return args[0].compute(app, source, true, true, full_range);
            } else {
                return (args[0].compute(app, source, true, true, full_range))[0][0];
            }
        }
    };
